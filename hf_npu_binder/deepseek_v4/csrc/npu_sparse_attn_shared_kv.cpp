...
//   mindspeed/ops/csrc/cann/npu_sparse_attn_shared_kv.cpp
//
// Changes vs upstream:
//   * include of aclnn_common.h: shifted from "inc/aclnn_common.h" to
//     the bare filename, since this file lives in hf_npu_binder's
//     deepseek_v4/csrc/ while the shared ACLNN dispatcher lives in
//     hf_npu_binder/shared/csrc/inc/ (passed via include_paths from our
//     op_builder so the bare name resolves).
//
// The 3 functions below (metadata / forward / grad) are thin glue around
// CANN's aclnn API — the actual NPU kernel lives in CANN's libopapi.so
// (resolved at first call via dlsym from aclnn_common.h's GET_OP_API_FUNC
// macro). Vendoring this file alone is safe because the heavy lifting is
// in CANN, and CANN ships as part of every NPU dev env.
