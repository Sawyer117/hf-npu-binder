...
#   ``mindspeed_llm/tasks/models/transformer/deepseek4/mhc/sinkhorn_triton_kernel.py``.
# Only change vs upstream: this header. Kernels + Python launchers are
# byte-identical to MindSpeed so future bug-fixes apply via re-vendor
# (one-file overwrite).
#
# Hardcoded ``hc_mult = 4`` throughout (DSV4 paper config). Other
# multiplicities would need new kernels — the unrolled loops in part1
# assume exactly 4 streams.
