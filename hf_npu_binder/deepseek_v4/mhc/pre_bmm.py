...
#   ``mindspeed_llm/tasks/models/transformer/deepseek4/mhc/pre_bmm.py``.
# Byte-identical to upstream; future fixes apply via re-vendor.
#
# Computes ``(H_pre.unsqueeze(-1) * x).sum(dim=2)`` for the alloy MHC
# HyperConnection's "pre" stage (single-stream collapse via learned pre
# weights). **Hardcoded N = 4** (= hc_mult = 4, DSV4 paper config) —
# the unrolled h0/h1/h2/h3 loads in the kernel body assume exactly 4
# streams. Other stream counts raise ValueError in the launcher.
import torch
